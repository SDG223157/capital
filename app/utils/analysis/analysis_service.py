            ...
            try:
                ...
                def evaluate_trend_score(quad_coef, linear_coef, r_squared):
                    try:
                        ...
                        # Calculate ratio and amplify quadratic influence
                        ratio = abs(quad_coef / linear_coef) if linear_coef != 0 else float('inf')
                        amplified_ratio = ratio * 2  # Double the quadratic influence
                        
                        # Base weight total is 50 points
                        total_weight = 50
                        
                        # Adjust weights based on amplified ratio
                        if ratio > 1:  # Quadratic dominates
                            quad_weight = total_weight * (amplified_ratio / (1 + amplified_ratio))
                            linear_weight = total_weight * (1 / (1 + amplified_ratio))
                        else:  # Linear dominates
                            # Keep original ratio for linear dominance
                            strength_factor = min(1/ratio, 2)
                        ...
                        strength_multiplier = 0.5 + (0.5 * math.pow(r_squared, 2))
